...
export function getNodeEncounterPreview(node: BoardNode): {
  ...
  if (node.type === 'boss' || node.type === 'dark_gate') {
    const bossMonster: MonsterProfile = {
      name: node.type === 'dark_gate' ? 'Altar of Rico (Darkling Queen)' : 'Dragon Princess Ignis',
    ...
    return {
      ...
      threatLevel: 'BOSS',
      threatColor: '#f43f5e',
      threatDescription: 'สมรภูมิชี้ชะตากับเจ้าหญิงมังกรเพลิงบรรพกาล! เสี่ยงตายขั้นสุดยอด!',
